...
#define ROUND_UP(x, s) (((x)+((s)-1)) & -(s))
	...
	void Matrix<T>::fill_rand(float bias) {
		for (int i = 0; i<this->height; i++) {
			int ai = i*phy_width;
			for (int j = 0; j<this->width; j++)
				matrix[ai + j] = bias + (float(rand() / 100.0));
		}
	...
	void Matrix<T>::fill_zeros() {
		memset(matrix, 0, sizeof(T)*phy_height*phy_width);
	}



	template<typename T>
	void Matrix<T>::transpose(Matrix<T>* res) const {
		for (int i = 0; i<this->height; i++)
		{
			int ai = i*phy_width;
			for (int j = 0; j<this->width; j++)
	...
	template<typename T>
	void Matrix<T>::mult1(Matrix<T>& B, Matrix<T>* output) {
		output->fill_zeros();
	...
	template<typename T>
	void Matrix<T>::mult2(const Matrix<T>& B, Matrix<T>* output) {
	...
	template<typename T>
	void Matrix<T>::mult3(const Matrix<T>& B, Matrix<T>* output) {
		output->fill_zeros();
		matmul_avx_8x8(this->matrix, B.matrix, output->matrix, this->phy_height, this->phy_width,
			B.phy_width, this->phy_width, B.phy_width, output->phy_width);
	}
	template<typename T>
	...
	template<typename T>
	void Matrix<T>::mult5(Matrix<T>& B, Matrix* output) {
		__m256 ymm0, ymm1, ymm2, ymm3, ymm4, ymm5, ymm6, ymm7,
			ymm8, ymm9, ymm10, ymm11, ymm12, ymm13, ymm14, ymm15;
		float scratchpad[8];

		const int col = width, row = height;
		...
		for (int z = 0; z < B.height; z++) {
			...
			for (int i = 0; i < row; i++) {
				...
				for (int j = 0; j < col_reduced; j += 64) {
					ymm8 = _mm256_loadu_ps(&x[j]);
					ymm9 = _mm256_loadu_ps(&x[j + 8]);
					ymm10 = _mm256_loadu_ps(&x[j + 16]);
					ymm11 = _mm256_loadu_ps(&x[j + 24]);
					ymm12 = _mm256_loadu_ps(&x[j + 32]);
					ymm13 = _mm256_loadu_ps(&x[j + 40]);
					ymm14 = _mm256_loadu_ps(&x[j + 48]);
					ymm15 = _mm256_loadu_ps(&x[j + 56]);

					ymm0 = _mm256_loadu_ps(&(*this)[i][j]);
					ymm1 = _mm256_loadu_ps(&(*this)[i][j + 8]);
					ymm2 = _mm256_loadu_ps(&(*this)[i][j + 16]);
					ymm3 = _mm256_loadu_ps(&(*this)[i][j + 24]);
					ymm4 = _mm256_loadu_ps(&(*this)[i][j + 32]);
					ymm5 = _mm256_loadu_ps(&(*this)[i][j + 40]);
					ymm6 = _mm256_loadu_ps(&(*this)[i][j + 48]);
					ymm7 = _mm256_loadu_ps(&(*this)[i][j + 56]);
					...
					_mm256_storeu_ps(scratchpad, ymm0);
					for (int k = 0; k < 8; k++)
						res += scratchpad[k];
				}
				for (int j = col_reduced; j < col_reduced_32; j += 32) {
					...
					_mm256_store_ps(scratchpad, ymm0);
					for (int k = 0; k < 8; k++)
						res += scratchpad[k];
				}
